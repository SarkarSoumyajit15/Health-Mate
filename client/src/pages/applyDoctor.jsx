...
function ApplyDoctor() {
  ...
  const handleDeleteTiming = (index) => {
    console.log(index);
    const allTimingsTemp = doctorInfo.allTimings;
  ...
  const handleSubmit = async (e) => {
    try {
      ...
      // if(isDisabled) {
      //   const testResponse = await axiosInstance.post("/api/testing/timing-test",doctorInfo,{withCredentials:true});
      //   return;
      // }
      console.log("submiting");

      const response = await axiosInstance.post(
        "/api/users/apply-doctor",
        doctorInfo,
        { withCredentials: true }
      );

      if (response?.data?.success) {
        toast.success("Succesfully Applied for doctors account");
        navigate("/");
      }
    } catch (err) {
      toast.error(
        `Doctors Account application failed : ${err?.response?.data?.message}`
      );
    }
  };
            ...
            doctorInfo.allTimings.map((timings, index) => (
              <div className="flex justify-between items-center p-2">
                ...
                <div>
                  ...
                  >
                    {isOpen ? (<i class="ri-delete-bin-5-line"></i>):(<i class="ri-delete-bin-line"></i>)}
                  </button>
                </div>
              </div>
            ))}
